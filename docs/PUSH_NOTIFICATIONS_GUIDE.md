...
```bash
curl -X POST http://localhost:10000/api/notifications/register-device \
...
```bash
curl http://localhost:10000/api/notifications/registered-devices
```

### Simulate Earthquake Alert Broadcast

```bash
curl -X POST http://localhost:10000/api/earthquake-alerts/dispatch
```
...
- **Cause**: Devices not registered (check stored file)
  - Verify: `curl http://localhost:10000/api/notifications/registered-devices`
