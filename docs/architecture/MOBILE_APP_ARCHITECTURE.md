...
- **Backend**: Node.js/Express on your API host (e.g. AWS)
...
User Makes Change
    ↓
App Calls API (https://<your-api-host>)
    ↓
...
4. **Server Monitoring**: Your API host’s logs and metrics (e.g. AWS CloudWatch)
...
### API Calls Failing
1. Verify backend running: `curl https://<your-api-host>/health`
2. Check network config: Review `network_security_config.xml`
3. Test with curl: `curl -v https://<your-api-host>/api/alerts/earthquakes`
