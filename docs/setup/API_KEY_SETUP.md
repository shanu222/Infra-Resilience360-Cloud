...
4. **Material Hub Portal** - Uses main backend via `VITE_PORTAL_STATIC_BASE_URL`
...
**File**: `server/index.mjs` (port 10000)
...
**Uses**: Main backend (`VITE_PORTAL_STATIC_BASE_URL`)

**Connection**:
- Configure `VITE_PORTAL_STATIC_BASE_URL` or rely on same-origin routing; endpoint is `POST /api/material-hubs/ai-agent` on your deployed API host (no baked-in URL).
...
For production deployment of the main Node API (e.g. on AWS):
...
2. **Set API host environment variables** (for main backend):
   ```
   OPENAI_API_KEYS=sk-key-1,sk-key-2,sk-key-3
   ...
   AI_PROVIDER=openai
   OPENAI_FALLBACK_TO_HUGGINGFACE=false
   CORS_ORIGIN=https://www.infraresilience.org
   ```
...
### Test Main Backend
```bash
curl -X POST http://localhost:10000/api/guidance/construction \
