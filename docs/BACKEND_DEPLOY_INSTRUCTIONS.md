...
## Option A: Node host (e.g. AWS ECS, EC2, Elastic Beanstalk)

### 1) Create service
- Provision a Node-capable HTTPS service and connect GitHub repo: shanu222/Resilience360
- Branch: main
...
Set these in your host’s environment UI (or parameter store):
- AI_PROVIDER=openai
...
- PORT is usually provided by the host (default 8787 for local / containers)

For portal credential recovery email (PGBC), add:
...
### 5) Verify
- Open: `https://<your-api-host>/health`
- Should return healthy response before wiring frontend calls
- Test recovery API with POST `https://<your-api-host>/api/recovery/send-credentials`

## Option B: Alternate PaaS

### 1) New project
- Create a project from this GitHub repo using your provider’s flow
- Select branch main
...
### 4) Auto-deploy
- Your CI/CD redeploys automatically on main branch pushes

### 5) Verify
- Test `https://<your-api-host>/health`
