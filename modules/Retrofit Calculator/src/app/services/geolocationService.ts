...
import { buildApiUrl, fetchApi, LOCATION_UNAVAILABLE } from '@resilience/api-base'
...
/**
 * Get approximate location from IP via same-origin API (avoids browser CORS to ipapi.co).
 */
export async function getLocationFromIP(): Promise<GeolocationResult> {
  try {
    const response = await fetchApi(buildApiUrl('/api/location'))
    ...
    console.warn('[geolocation] IP lookup failed, using Lahore default:', error)
    return {
      city: 'Lahore',
      country: 'Pakistan',
      latitude: 31.5204,
