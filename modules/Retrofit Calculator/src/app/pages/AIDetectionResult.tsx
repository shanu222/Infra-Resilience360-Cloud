...
export function AIDetectionResult() {
  ...
  const generateAnnotationSummary = () => {
    ...
    if (replacementRecommended) {
      setAnnotationInsight(r.insight_severe)
    } else if (investigationRequired) {
      setAnnotationInsight(r.insight_large)
    } else if (damagePercent > 0) {
      setAnnotationInsight(r.insight_localized)
    } else {
      setAnnotationInsight(r.insight_none)
    }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 sm:py-6">
          ...
          >
            ...
            <div className="flex items-center gap-2 px-4 py-2 bg-green-50 text-green-700 border border-green-200 rounded-lg">
              ...
              <span className="text-sm font-medium">{r.ai_detectionComplete}</span>
            </div>
          </motion.div>
        </div>
      </div>
          ...
          <div className="xl:col-span-2 space-y-6">
            ...
            >
              <div className="relative select-none" style={{ touchAction: "none" }}>
                ...
                <div className="absolute top-4 right-4 bg-slate-900/80 text-white px-3 py-1.5 rounded-lg text-xs font-medium shadow-lg backdrop-blur">
                  {r.ai_damageMarked} {damagePercentPreview}%
                </div>
              </div>
              ...
              <div className="px-4 py-4 border-t border-slate-100 bg-slate-50/70 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <label className="text-xs text-slate-600">
                    {r.ai_brushOpacity} ({Math.round(brushOpacity * 100)}%)
                    <input
                      type="range"
                      min={20}
                      max={100}
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-xs text-slate-600 inline-flex items-center gap-1.5">
                    ...
                    {r.ai_activeBrush} <strong style={{ color: activeBrush.color }}>{activeBrush.label}</strong>
                  </span>
                </div>
                  ...
                  <div className="overflow-x-auto border border-slate-200 rounded-lg">
                    <table className="w-full text-xs">
                      <thead className="bg-white border-b border-slate-200">
                        <tr>
                          <th className="text-left px-3 py-2 font-semibold text-slate-600">{r.ai_severity}</th>
                          <th className="text-right px-3 py-2 font-semibold text-slate-600">{r.ai_areaPct}</th>
                          <th className="text-right px-3 py-2 font-semibold text-slate-600">{r.ai_areaM2}</th>
                        </tr>
                      </thead>
            ...
            >
              <div className="flex items-center gap-2 mb-5">
                ...
                <h3 className="text-[16px] font-semibold text-[#0F172A]">{r.ai_summaryTitle}</h3>
              </div>
              
              <div className="space-y-4">
                <div className="flex justify-between items-center pb-4 border-b border-slate-100">
                  <span className="text-sm text-slate-600">{r.ai_elementType}</span>
                  <span className="px-3 py-1.5 bg-[#2563EB] text-white rounded-lg text-sm font-medium">
                    {detectionData?.elementType ?? r.ai_structuralElement}
                  </span>
                </div>
                
                <div className="flex justify-between items-center pb-4 border-b border-slate-100">
                  <span className="text-sm text-slate-600">{r.ai_defectType}</span>
                  <span className="px-3 py-1.5 bg-red-500 text-white rounded-lg text-sm font-medium">
                    {detectionData?.defectType ?? r.ai_notDetected}
                  </span>
                </div>
                
                <div className="flex justify-between items-center pb-4 border-b border-slate-100">
                  <span className="text-sm text-slate-600">{r.ai_severityLevel}</span>
                  <span className={`px-3 py-1.5 ${severityTone} text-white rounded-lg text-sm font-medium flex items-center gap-1.5`}>
                    <AlertCircle className="w-3.5 h-3.5" />
                    {detectionData?.severity ?? r.ai_moderate}
                  </span>
                </div>
                
                <div className="flex justify-between items-center">
                  <span className="text-sm text-slate-600">{r.ai_confidence}</span>
              ...
              <div className="space-y-8">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {[
                      { label: r.ai_widthCm, value: formData.widthCm, key: "widthCm" },
                      { label: r.ai_depthCm, value: formData.depthCm, key: "depthCm" },
                      { label: r.ai_heightCm, value: formData.heightCm, key: "heightCm" },
                    ].map((field) => (
                      <div key={field.key}>
                ...
                <div>
                  ...
                  >
                    <option value="Reinforced Concrete">{r.ai_mat_rc}</option>
                    <option value="Brick Masonry">{r.ai_mat_brick}</option>
                    <option value="Steel">{r.ai_mat_steel}</option>
                    <option value="Block">{r.ai_mat_block}</option>
                    <option value="Adobe">{r.ai_mat_adobe}</option>
                  </select>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:flex gap-3">
                    {["Ground", "1", "2", "3", "4+"].map((level) => (
                      ...
                      >
                        {level === "Ground" ? r.ai_fl_ground : level === "4+" ? r.ai_fl_plus : level}
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {[
                      { key: "tightAccess", label: r.ai_tightAccess },
                      { key: "occupied", label: r.ai_occupied },
                      { key: "scaffolding", label: r.ai_scaffolding },
                    ].map((item) => (
                      <label
                        key={item.key}
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {[
                      { value: "cosmetic", label: r.ai_retro_cosmetic, desc: r.ai_retro_cosmetic_d },
                      { value: "structural", label: r.ai_retro_struct, desc: r.ai_retro_struct_d },
                      { value: "seismic", label: r.ai_retro_seismic, desc: r.ai_retro_seismic_d },
                    ].map((option) => (
                      <label
                        key={option.value}
