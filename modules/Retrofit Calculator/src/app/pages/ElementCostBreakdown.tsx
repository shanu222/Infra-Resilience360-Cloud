...
export function ElementCostBreakdown() {
  ...
  const deterministicCostItems = useMemo(() => {
    ...
    return [
      { item: r.cost_item_surface, quantity: `${dimensions.surfaceAreaM2.toFixed(2)} m²`, unitCost: surfacePrepRate, total: surfacePreparation, icon: Package },
      { item: r.cost_item_epoxy, quantity: `${dimensions.crackLengthM.toFixed(2)} m`, unitCost: epoxyRate, total: epoxyInjection, icon: Package },
      { item: r.cost_item_rc, quantity: `${dimensions.jacketVolumeM3.toFixed(2)} m³`, unitCost: rcJacketRate, total: rcJacketing, icon: Package },
      { item: r.cost_item_labor, quantity: `${Math.round(32 + formData.damageExtent * 0.8)} hrs`, unitCost: laborRate, total: skilledLabor, icon: Users },
    ]
  }, [dimensions.crackLengthM, dimensions.jacketVolumeM3, dimensions.surfaceAreaM2, formData.damageExtent, cityRates, r])
  ...
  const annotationCostItems = useMemo(() => {
    ...
    if (manualAnnotation.investigationRequired) {
      ...
      severityRows.push({
        item: r.cost_item_investigation,
        quantity: r.cost_qty_lump,
    ...
    return severityRows
  }, [complexityMultiplier, locationMultiplier, manualAnnotation, cityRates, r])
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 sm:py-6">
          ...
          >
            ...
            <div className="flex items-center gap-2 px-4 py-2 bg-green-50 text-green-700 border border-green-200 rounded-lg">
              ...
              <span className="text-sm font-medium">{r.cost_confidenceBadge}</span>
            </div>
          </motion.div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        >
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h2 className="text-[20px] font-semibold text-[#0F172A]">{detectionData?.elementType ?? "Element"} {r.cost_estimateTitle}</h2>
              <p className="text-sm text-slate-500">{r.cost_estimateSub}</p>
            </div>
          </div>
          ...
          {calculating ? (
            ...
            >
              ...
              <p className="text-slate-600 text-sm">{r.cost_calculating}</p>
            </motion.div>
          ) : (
            <>
              {/* Line Items Table */}
              <div className="border border-slate-200 rounded-lg overflow-x-auto mb-6">
                <table className="w-full">
                  <thead className="bg-slate-50 border-b border-slate-200">
                    <tr>
                      <th className="text-left px-6 py-3 text-sm font-semibold text-slate-700">{r.cost_colItem}</th>
                      <th className="text-left px-6 py-3 text-sm font-semibold text-slate-700">{r.cost_colQty}</th>
                      <th className="text-left px-6 py-3 text-sm font-semibold text-slate-700">{r.cost_colUnit}</th>
                      <th className="text-right px-6 py-3 text-sm font-semibold text-slate-700">{r.cost_colTotal}</th>
                    </tr>
                  </thead>
                  ...
                  <tfoot className="bg-slate-50 border-t border-slate-200">
                    <tr>
                      <td colSpan={3} className="px-6 py-4 text-sm font-semibold text-slate-700">{r.cost_baseSubtotal}</td>
              ...
              {manualAnnotation && manualAnnotation.zones.length > 0 && (
                <div className="border border-slate-200 rounded-lg overflow-x-auto mb-6">
                  <table className="w-full">
                    <thead className="bg-slate-50 border-b border-slate-200">
                      <tr>
                        <th className="text-left px-6 py-3 text-sm font-semibold text-slate-700">{r.cost_severityLayer}</th>
                        <th className="text-right px-6 py-3 text-sm font-semibold text-slate-700">{r.cost_areaPct}</th>
                        <th className="text-right px-6 py-3 text-sm font-semibold text-slate-700">{r.cost_areaM2}</th>
                        <th className="text-left px-6 py-3 text-sm font-semibold text-slate-700">{r.cost_strategy}</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
              ...
              >
                <div className="flex items-center gap-2 mb-5">
                  ...
                  <h3 className="text-[16px] font-semibold text-[#0F172A]">{r.cost_summaryTitle}</h3>
                </div>
                <div className="space-y-3">
                  <div className="flex justify-between items-center p-3 bg-white rounded-lg">
                    <span className="text-sm text-slate-600">{r.cost_baseRepair}</span>
                    <span className="text-sm font-semibold text-slate-900">PKR {baseCost.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-white rounded-lg">
                    <span className="text-sm text-slate-600">{r.cost_locFactor} ({location})</span>
                    <span className="text-sm font-semibold text-[#2563EB]">×{locationMultiplier}</span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-white rounded-lg">
                    <span className="text-sm text-slate-600">{r.cost_complexity}</span>
                    <span className="text-sm font-semibold text-[#2563EB]">×{complexityMultiplier.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-white rounded-lg">
                    <span className="text-sm text-slate-600">{r.cost_retrofitFactor}</span>
                    <span className="text-sm font-semibold text-[#2563EB]">×{retrofitLevelFactor.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-blue-50 rounded-lg border-2 border-blue-200">
                    <span className="text-sm font-medium text-slate-700">{r.cost_adjSubtotal}</span>
                    <span className="text-sm font-bold text-[#2563EB]">PKR {adjustedSubtotal.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-white rounded-lg">
                    <span className="text-sm text-slate-600">{r.cost_contingency}</span>
                  ...
                  <div className="flex justify-between items-center p-3 bg-white rounded-lg border-b-2 border-slate-200 pb-4">
                    <span className="text-sm text-slate-600">{r.cost_overhead}</span>
                  ...
                  <div className="flex justify-between items-center pt-2 p-4 bg-gradient-to-r from-[#2563EB] to-[#1D4ED8] rounded-lg shadow-sm">
                    <span className="text-white text-base font-medium">{r.cost_totalEst}</span>
                    <span className="text-white text-2xl font-bold">PKR {totalCost.toLocaleString()}</span>
                  </div>
                </div>
              </motion.div>
              {mlError && (
                <p className="text-xs text-amber-700 mb-4">{r.cost_mlFallback} {mlError}. {r.cost_mlFallbackEnd}</p>
              )}
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium">{r.cost_methodology}</span>
                </div>
              ...
              {showCalculation && (
                ...
                >
                  <h4 className="text-[15px] font-semibold text-[#0F172A] mb-4">{r.cost_howTitle}</h4>
                  <div className="space-y-3 text-sm text-slate-700">
                    <p className="p-3 bg-white rounded-lg">
                      <strong>{r.cost_how1}</strong>{r.cost_how1b}
                    </p>
                    <p className="p-3 bg-white rounded-lg">
                      <strong>{r.cost_how2}</strong>{r.cost_how2b}
                    </p>
                    <p className="p-3 bg-white rounded-lg">
                      <strong>{r.cost_how3}</strong>{r.cost_how3b}
                    </p>
                    <p className="p-3 bg-white rounded-lg">
                      <strong>{r.cost_how4}</strong>{r.cost_how4b}
                    </p>
                    <p className="p-3 bg-white rounded-lg">
                      <strong>{r.cost_how5}</strong>{r.cost_how5b}
                    </p>
                  </div>
                </motion.div>
              )}
