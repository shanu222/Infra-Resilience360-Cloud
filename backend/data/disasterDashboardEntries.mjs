/**
 * Canonical EN defaults for Disaster Dashboard CMS (mirrors portal `disasters.ts` + seed script).
 * Shared by `scripts/seed-disaster-dashboard-pages.mjs` and runtime auto-seed.
 */

export const mapGuidance = (g) => ({
  before: (g.before || []).map((text) => ({ text, image: '', video: '', audio: '' })),
  during: (g.during || []).map((text) => ({ text, image: '', video: '', audio: '' })),
  after: (g.after || []).map((text) => ({ text, image: '', video: '', audio: '' })),
})

/** Mirrors `Disaster Dashboard UX Flow/src/app/data/disasters.ts` — English defaults. */
