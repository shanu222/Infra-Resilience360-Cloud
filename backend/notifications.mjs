...
export async function initializeNotificationSystem() {
  try {
    ...
    console.error('[notifications] Failed to initialize notification system.')
  }
}
...
export async function readRegisteredDevices() {
  try {
    ...
    console.error('[notifications] Failed to read registered devices.')
    return []
  }
}
...
export async function writeRegisteredDevices(devices) {
  try {
    ...
    console.error('[notifications] Failed to write registered devices.')
    return false
  }
}
...
export async function registerDevice(deviceToken, platform = 'android') {
  try {
    ...
    console.info('[notifications] Device registered.')
...
export async function unregisterDevice(deviceToken) {
  try {
    ...
    console.info('[notifications] Device unregistered.')
    return { ok: true, total: remaining.length }
  } catch (error) {
    console.error('[notifications] Failed to unregister device.')
    return { ok: false, error: error.message }
  }
}
...
export async function updateSubscriptionPreferences(deviceToken, preferences) {
  try {
    ...
    await writeRegisteredDevices(devices)
    console.info('[notifications] Updated subscription preferences.')
    return { ok: true, subscriptions: device.subscriptions }
  } catch (error) {
    console.error('[notifications] Failed to update subscriptions.')
    return { ok: false, error: error.message }
  }
}
...
export async function getDevicesForBroadcast(options = {}) {
  try {
    ...
    console.error('[notifications] Failed to get devices for broadcast.')
    return []
  }
}
