...
export type RiskMapUiLabels = {
  provinceView: string
  backToProvinces: string
  districtViewPrefix: string
  popupProvince: string
  popupType: string
  popupSeverity: string
  layerEarthquake: string
  layerFlood: string
  layerInfraRisk: string
  hazard: string
  year: string
...
const defaultRiskMapUiLabels: RiskMapUiLabels = {
  provinceView: 'Province View',
  backToProvinces: '↩️ Back to Provinces',
  districtViewPrefix: 'District View:',
  popupProvince: 'Province',
  popupType: 'Type',
  popupSeverity: 'Severity',
  layerEarthquake: 'Earthquake',
  layerFlood: 'Flood',
  layerInfraRisk: 'Infrastructure Risk',
  hazard: 'Hazard',
  year: 'Year',
  estimatedExtent: 'Estimated extent',
  livesLost: 'Lives lost',
  peopleAffected: 'People affected',
  economicCost: 'Economic cost',
  source: 'Source',
  depthKm: 'Depth:',
...
type RiskMapProps = {
  ...
  /** When set, overrides default English map chrome and popups */
  uiLabels?: Partial<RiskMapUiLabels>
}
...
function RiskMap({
  ...
  const L = useMemo(() => ({ ...defaultRiskMapUiLabels, ...uiLabels }), [uiLabels])
  const layerLabel = useMemo(() => {
    if (layer === 'earthquake') return L.layerEarthquake
    if (layer === 'flood') return L.layerFlood
    return L.layerInfraRisk
  }, [layer, L.layerEarthquake, L.layerFlood, L.layerInfraRisk])
  ...
  return (
    <>
      <div className="map-toolbar">
        {!inDistrictView && <span className="map-pill">{L.provinceView}</span>}
        {inDistrictView && (
          <>
            <button
              className="map-back"
            ...
            <span className="map-pill">
              {L.districtViewPrefix} {drillProvince}
            </span>
          </>
        )}
      </div>
        ...
        {!inDistrictView && mapping.provinceCollection && (
          <GeoJSON
            ...
            onEachFeature={(feature: Feature<Geometry, Adm1Props>, layerRef) => {
              ...
              layerRef.bindPopup(`<strong>${shapeName}</strong><br/>${layerLabel}: ${risk}`)
            }}
          />
        )}
        ...
        {inDistrictView && districtCollection && (
          <GeoJSON
            ...
            onEachFeature={(feature: Feature<Geometry, Adm2Props>, layerRef) => {
              ...
              layerRef.bindPopup(
                `<strong>${districtName}</strong><br/>${L.popupProvince}: ${drillProvince}<br/>${layerLabel}: ${districtRisk}`,
              )
            }}
          />
        )}
        ...
        {(alertMarkers ?? []).map((alert) => (
          ...
          >
            ...
            <Popup>
              ...
              <br />
              {L.popupType}: {alert.type}
              <br />
              {L.popupSeverity}: {alert.severity}
              <br />
              {alert.advisory}
            </Popup>
          </CircleMarker>
          ...
          return (
            ...
            >
              ...
              <Popup>
                ...
                <br />
                {L.hazard}: {event.hazard}
                <br />
                {L.year}: {event.year}
                <br />
                {L.estimatedExtent}: {event.extentKm} {L.kmRadius}
                <br />
                {L.livesLost}: {event.livesLost.toLocaleString()}
                <br />
                {event.affectedPeopleMillions !== undefined && (
                  <>
                    {L.peopleAffected}: {event.affectedPeopleMillions.toFixed(1)} million
                    <br />
                  </>
                )}
                {event.economicCostUsdBn !== undefined && (
                  <>
                    {L.economicCost}: ${event.economicCostUsdBn.toFixed(1)} bn
                    <br />
                  </>
                )}
            ...
            return (
              ...
              >
                ...
                <Popup>
                  ...
                  {quake.place}
                  <br />
                  {L.depthKm} {quake.depthKm.toFixed(1)} km
                  <br />
