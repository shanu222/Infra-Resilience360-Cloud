...
const fetchJsonWithFallback = async <T>(path: string, init?: RequestInit): Promise<T> => {
  ...
  for (const target of targets) {
    try {
      const response = await fetchApi(target, init)
...
export const syncSharedInfraModelsToGitHub = async () =>
  fetchJsonWithFallback<{ committed: boolean; pushed: boolean; branch?: string; message: string }>(
