...
import { RESILIENCE360_STATIC_LOCAL_BASE } from '../config/localContent'

const LOCAL_INFRA_MEDIA_BASE = `${RESILIENCE360_STATIC_LOCAL_BASE}/infra-models`
const LOCAL_INFRA_RESILIENCE_BASE = `${RESILIENCE360_STATIC_LOCAL_BASE}/infra-resilience`

const practicalModelImagePool = [
  `${LOCAL_INFRA_RESILIENCE_BASE}/flood-housing-cluster-pk/flood-housing-cluster-pk.jpg`,
  `${LOCAL_INFRA_MEDIA_BASE}/flood-health-post-pk/flood-health-post-pk.jpg`,
  `${LOCAL_INFRA_MEDIA_BASE}/lifeline-utility-corridor-pk/lifeline-utility-corridor-pk.jpg`,
  `${LOCAL_INFRA_MEDIA_BASE}/sponge-street-drainage-pk/sponge-street-drainage-pk.jpg`,
  `${LOCAL_INFRA_MEDIA_BASE}/resilient-waterpoint-pk/resilient-waterpoint-pk.jpg`,
]

function mergedFallbackImagePool(): string[] {
  const merged = [...practicalModelImagePool].filter(Boolean)
...
const buildPracticalFallbackModels = (count = 4, imagePool: string[] = practicalModelImagePool): InfraModel[] => {
  const seed = Date.now()
  ...
  return selected.map((model, index) => ({
    ...
    imageDataUrl: imagePool[(start + index) % imagePool.length] ?? '',
  }))
}

const generateImagesFromGuidanceApi = async (payload: { country?: string; province?: string }) => {
  const imagePool = mergedFallbackImagePool()
  try {
    ...
    return fallbackModelSpecs.map((item, index) => {
      ...
      return {
        ...
        imageDataUrl: matched?.imageDataUrl ?? imagePool[index % imagePool.length] ?? '',
      }
    })
  } catch {
    return buildPracticalFallbackModels(4, imagePool)
  }
}
...
export const fetchResilienceInfraModels = async (payload: {
  ...
  try {
    ...
    if (!Array.isArray(parsed.models) || parsed.models.length === 0) {
      return { models: buildPracticalFallbackModels(4, imagePool) }
    }
    ...
    return {
      models: parsed.models.map((model, index) => ({
        ...
        imageDataUrl: model.imageDataUrl || imagePool[index % imagePool.length] || '',
      })),
    }
  } catch {
