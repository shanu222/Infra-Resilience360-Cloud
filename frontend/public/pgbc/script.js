...
function resolvePgbcPdfPath(localPath) {
    if (typeof window.R360ContentMedia !== 'undefined' && typeof window.R360ContentMedia.resolvePgbcPdfPath === 'function') {
        return window.R360ContentMedia.resolvePgbcPdfPath(localPath);
    }
    return String(localPath || '').trim();
}

function normalizeCodeMediaPaths(code) {
    if (!code || typeof code !== 'object') return code;
    const next = { ...code };
    if (next.pdfPath) next.pdfPath = resolvePgbcPdfPath(next.pdfPath);
    if (next.pdf) next.pdf = resolvePgbcPdfPath(next.pdf);
    return next;
}
...
function portalNavigate(path) {
    if (typeof window.appendLangToPath === 'function') {
        return window.appendLangToPath(path);
    }
    return path;
}
...
async function searchLibrary() {
    ...
    window.location.href = portalNavigate('code-viewer.html');
}
...
predefinedCodes.forEach(code => {
    code.chapters = [];
    if (code.pdfPath) code.pdfPath = resolvePgbcPdfPath(code.pdfPath);
});
...
const PGBC_ADMIN_SECTION_DISABLED = true;

function showSection(id) {
    if (PGBC_ADMIN_SECTION_DISABLED && id === 'adminLogin') {
        alert('Admin section is disabled on this public portal.');
        id = 'home';
    }
    // Hide all sections
...
async function handleLogin() {
    ...
    if (supabaseClient) {
        try {
            ...
            updateNavButtons();
            window.location.href = portalNavigate('library.html');
            return;
        } catch (error) {
...
function handleAdminLogin() {
    if (PGBC_ADMIN_SECTION_DISABLED) {
        alert('Admin section is disabled on this public portal.');
        showSection('home');
        return;
    }
...
function logout() {
    ...
    if (userInfo) {
        userInfo.innerText = "Public Access";
    }
    showSection('home');
}
...
let availableCodesCache = [];

function extractCodeYear(name) {
    const matches = String(name || '').match(/\b(19|20)\d{2}\b/g);
    return matches && matches.length ? matches[matches.length - 1] : 'N/A';
}

function buildCodeListItems() {
    const items = [];
    ...
    predefinedCodes.forEach((code, index) => {
        ...
        items.push({
            ...
            index,
            name: code.name,
            year: extractCodeYear(code.name),
            type: 'Predefined',
            pdfPath: code.pdfPath || code.pdf
        });
    });
...
function initializeCodeListControls() {
    const searchInput = document.getElementById('codeListSearchInput');
    const yearFilter = document.getElementById('codeYearFilter');
    if (!searchInput || !yearFilter) return;

    const years = Array.from(new Set(availableCodesCache.map(code => code.year))).sort((a, b) => {
        if (a === 'N/A') return 1;
        if (b === 'N/A') return -1;
    ...
    if (searchInput.dataset.bound !== '1') {
        searchInput.addEventListener('input', applyCodeListFilters);
        searchInput.dataset.bound = '1';
    }

    if (yearFilter.dataset.bound !== '1') {
        yearFilter.addEventListener('change', applyCodeListFilters);
        yearFilter.dataset.bound = '1';
    }
}

function renderCodesListRows(items) {
    const list = document.getElementById('codesList');
    if (!list) return;

    if (!items.length) {
        list.innerHTML = '<div class="codes-empty-state">No matching codes found.</div>';
        return;
    }

    list.innerHTML = items.map(code => {
        const fullName = String(code.name || '');
        const safeName = escapeHtml(fullName);
        const pdfLabel = code.source === 'uploaded' ? 'PDF' : 'PDF';

        return `
            <div class="code-list-row">
                <div class="code-list-cell code-list-cell-select" onclick="event.stopPropagation()">
                    <input type="checkbox" name="qaCodeSelection" value="${code.key}" aria-label="Select ${safeName}">
                </div>
                <div class="code-list-cell code-list-cell-title" title="${safeName}">
                    <div class="code-row-title">${safeName}</div>
                    <div class="code-row-meta">${code.type} Code</div>
                </div>
                <div class="code-list-cell code-list-cell-year">${code.year}</div>
                <div class="code-list-cell code-list-cell-actions" onclick="event.stopPropagation()">
                    <button type="button" class="code-action-btn view" onclick="openCodeViewer('${code.source}', ${code.index})">🌐 Open Code</button>
                    <a href="${code.pdfPath}" download="${safeName}.pdf" onclick="event.stopPropagation()">
                        <button type="button" class="code-action-btn pdf">📄 ${pdfLabel}</button>
                    </a>
                    ${currentUser?.role === 'Authority' ? `<button type="button" class="code-action-btn danger" onclick="delete${code.source === 'predefined' ? 'Predefined' : 'Uploaded'}Code(${code.index})">🗑️</button>` : ''}
                </div>
            </div>
        `;
    }).join('');
...
function applyCodeListFilters() {
    const searchInput = document.getElementById('codeListSearchInput');
    const yearFilter = document.getElementById('codeYearFilter');

    const searchTerm = normalizeSearchText(searchInput?.value || '');
    const selectedYear = String(yearFilter?.value || 'all');

    const filtered = availableCodesCache.filter(code => {
        const matchesSearch = !searchTerm || normalizeSearchText(code.name).includes(searchTerm);
        const matchesYear = selectedYear === 'all' || code.year === selectedYear;
        return matchesSearch && matchesYear;
    });

    renderCodesListRows(filtered);
    initializeCodeQaPanel();
}

function loadCodes() {
    availableCodesCache = buildCodeListItems();
    initializeCodeListControls();
    applyCodeListFilters();
}
...
function initializeCodeQaPanel() {
    const qaPanel = document.getElementById('codeQaPanel');
    if (!qaPanel) return;

    const selectAllCheckbox = document.getElementById('selectAllCodesCheckbox');
    const checkboxes = Array.from(document.querySelectorAll('input[name="qaCodeSelection"]'));
    ...
    if (selectAllCheckbox && selectAllCheckbox.checked) {
        ...
        const alreadySelected = checkboxes.find(checkbox => checkbox.checked);
        if (!alreadySelected) {
    ...
    checkboxes.forEach(checkbox => {
        checkbox.onchange = function() {
            const selected = Array.from(document.querySelectorAll('input[name="qaCodeSelection"]:checked'));
            if (selectAllCheckbox) {
                selectAllCheckbox.checked = selected.length > 0 && selected.length === checkboxes.length;
            }
            if (selected.length === 0) {
                updateCodeQaStatus('No code selected.');
            } else if (selected.length === 1) {
                updateCodeQaStatus(`Selected code: ${getCodeNameByKey(selected[0].value)}`);
            } else {
                updateCodeQaStatus(`${selected.length} codes selected.`);
            }
        };
    });
    ...
    if (!selectAllCheckbox || !selectAllCheckbox.checked) {
        ...
        updateCodeQaStatus('All codes selected.');
    }
}

function updateCodeQaStatus(message) {
...
function getCodeNameByKey(codeKey) {
    const matched = getAllAvailableCodesForQa().find(code => code.key === codeKey);
    return matched ? matched.name : 'Unknown code';
}

function toggleSelectAllCodes(isChecked) {
    const checkboxes = document.querySelectorAll('input[name="qaCodeSelection"]');
    if (isChecked) {
        checkboxes.forEach(checkbox => {
            checkbox.checked = true;
        });
...
function scoreCandidateText(text, tokens) {
...
async function answerQuestionFromSelectedCodes() {
    ...
    if (selectAllCheckbox?.checked) {
        ...
        const selectedBoxes = Array.from(document.querySelectorAll('input[name="qaCodeSelection"]:checked'));
        if (!selectedBoxes.length) {
            alert('Please select at least one code or choose "Select all codes".');
            return;
        }

        const selectedKeys = new Set(selectedBoxes.map(box => box.value));
        selectedCodes = allCodes.filter(code => selectedKeys.has(code.key));
    }

    if (!selectedCodes.length) {
        alert('No code is available for answering.');
        return;
    }

    updateCodeQaStatus(`Searching ${selectedCodes.length} selected code(s)...`);
    if (resultEl) resultEl.innerText = 'Finding the most relevant sections...';
    ...
    for (const code of selectedCodes) {
        ...
        chapters.forEach(chapter => {
            const chapterLabel = `Chapter ${chapter.number}: ${chapter.title}`;
            const chapterScore = scoreCandidateText(chapterLabel, tokens);
            if (chapterScore > 0) {
                candidates.push({
                    score: chapterScore,
                    text: chapterLabel
            ...
            (chapter.sections || []).forEach(section => {
                ...
                const sectionLabel = `${sectionCode ? `${sectionCode} ` : ''}${sectionTitle}`.trim();
                const fullText = `${chapterLabel} - ${sectionLabel}`;
                const sectionScore = scoreCandidateText(fullText, tokens);
                if (sectionScore > 0) {
                    candidates.push({
        ...
        candidates.forEach(item => {
            if (uniqueTop.length >= 5) return;
            const key = normalizeSearchText(item.text);
            if (seen.has(key)) return;
            seen.add(key);
            uniqueTop.push(item.text);
        });

        if (!uniqueTop.length) {
            answerBlocks.push(`${code.name}\n- No closely matched section heading found for this question. Use "View in App" to browse chapters directly.`);
        } else {
            answerBlocks.push(`${code.name}\n- ${uniqueTop.join('\n- ')}`);
        }
    }
    ...
    const finalAnswer = [
        `Question: ${question}`,
        '',
        'Most relevant sections from selected code(s):',
        '',
        answerBlocks.join('\n\n'),
        '',
        'Tip: Click "View in App" next to a code to open full in-app navigation and details.'
    ].join('\n');

    if (resultEl) resultEl.innerText = finalAnswer;
    updateCodeQaStatus('Answer ready.');
}
...
async function extractPdfText(pdfPath) {
    pdfPath = resolvePgbcPdfPath(pdfPath);
    if (!pdfPath) return '';
